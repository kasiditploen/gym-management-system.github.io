...
$query="INSERT INTO cf (cfid,image,cfName,description,brands,categories,vendors,amount,warranty) values('$cfid','$image','$name','$desc','$brand','$category','$vendor','$amount','$warranty')";
...
//$query="insert into users(username,gender,mobile,email,dob,joining_date,userid) values('$uname','$gender','$phn','$email','$dob','$jdate','$memID','$image')";
    if(mysqli_query($con,$query)==1){
      $query1="select * from cf where warranty = '$warranty'";
      
     
          if(mysqli_query($con,$query1)==1){

            echo "<head><script>alert(' Commercial Flooring Added ');</script></head></html>";
      echo "<meta http-equiv='refresh' content='0; url=view_cf.php'>";
              ...
              echo "error: ".mysqli_error($con);
               //Deleting record of users if inserting to enrolls_to table failed to execute
                $query3 = "DELETE FROM cf WHERE cfid='$cfid'";
