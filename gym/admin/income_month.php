...
e.paid_date,e.expire,p.planName,p.amount,p.validity
from users u 
...
union all
select distinct u.userid,u.username,u.gender,u.mobile,
u.email,u.joining_date,a.state,a.city,
s.paid_date,s.expire,p.planName,p.amount,p.validity
from users u 
INNER JOIN address a on u.userid=a.id 
INNER JOIN  sessions s on u.userid=s.userid
INNER JOIN plan p on p.pid=s.pid
where s.paid_date  like '".$year."-".$month."___' 
union all
select distinct u.userid,u.username,u.gender,u.mobile,
u.email,u.joining_date,a.state,a.city,
cs.paid_date,cs.expire,p.planName,p.amount,p.validity
from users u 
INNER JOIN address a on u.userid=a.id 
INNER JOIN csessions cs on u.userid=cs.userid
INNER JOIN plan p on p.pid=cs.pid
where cs.paid_date  like '".$year."-".$month."___' 
";
