...
$query="INSERT INTO se (seid,image,seName,description,brands,categories,vendors,amount,warranty) values('$seid','$image','$name','$desc','$brand','$category','$vendor','$amount','$warranty')";
...
//$query="insert into users(username,gender,mobile,email,dob,joining_date,userid) values('$uname','$gender','$phn','$email','$dob','$jdate','$memID','$image')";
    if(mysqli_query($con,$query)==1){
      $query1="select * from se where warranty = '$warranty'";
      
     
          if(mysqli_query($con,$query1)==1){

            echo "<head><script>alert('Strength Equipment Added ');</script></head></html>";
      echo "<meta http-equiv='refresh' content='0; url=view_se.php'>";
