...
?>
  <!-- Page wrapper  -->
        <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-primary">Member Per Day</h3> </div>
                <div class="col-md-7 align-self-center">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                        <li class="breadcrumb-item active">Member Per Day</li>
                    </ol>
                </div>
            </div>
            <!-- End Bread crumb -->
    ...
    <select name="day" id="sday">
        ...
        foreach ($formattedDayArray as $day) {
            // if you want to select a particular day
            $dd=implode(array_keys($formattedDayArray,$day));
            $selected = ($dd == date('d')) ? 'selected' : '';
            // if you want to add extra 0 before the month uncomment the line below
            //$day = str_pad($day, 2, "0", STR_PAD_LEFT);
            echo '<option '.$selected.' value="'.$dd.'">'.$day.'</option>';
        }
        ?>
    </select>
    ...
    <input type="button" class="btn btn-primary btn-flat m-b-30 m-t-30"  style="margin-bottom:5px;" name="search" onclick="showMember();" value="Search">
                                    ...
                                    <table id="memday" border=1 class="table table-bordered table-striped">
...
<script>
  ...
  function showMember(){
    ...
    var iday=day.selectedIndex;
    var dnumber=day.options[iday].value;
    var mnumber=month.options[imonth].value;
    var ynumber=year.options[iyear].value;
    if(mnumber=="0" || ynumber=="0" || dnumber=="0"){
      document.getElementById("memday").innerHTML="";
      return;
    }
    else{
        ...
        xmlhttp.onreadystatechange=function(){
            if(this.readyState==4 && this.status ==200){
                document.getElementById("memday").innerHTML=this.responseText;
            }
        };
        xmlhttp.open("GET","over_month.php?dd="+dnumber+"&mm="+mnumber+"&yy="+ynumber+"&flag=0",true);
        xmlhttp.send();
    }
