...
$(document).ready(function() {
	...
	$("#addProductModalBtn").unbind('click').bind('click', function() {
		...
		$("#submitProductForm").unbind('submit').bind('submit', function() {
			...
			if(image == "") {
				$("#image").closest('.center-block').after('<p class="text-danger">Product Image field is required</p>');
				$('#image').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#image").find('.text-danger').remove();
				// success out for form 
				$("#image").closest('.form-group').addClass('has-success');	  	
			}	// /else

			if(machineName == "") {
				$("#machineName").after('<p class="text-danger">Product Name field is required</p>');
				$('#machineName').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#machineName").find('.text-danger').remove();
				// success out for form 
				$("#machineName").closest('.form-group').addClass('has-success');	  	
			}	// /else
			...
			if(image && machineName && quantity && rate && brandName && categoryName && productStatus) {
	...
	if(productId) {
		...
		$.ajax({
			...
			// alert(response.product_image);
				...
				$("#getimage").attr('src', 'stock/'+response.product_image);
		  ...
		  // 		allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"]
				...
				// product name
				$("#editmachineName").val(response.product_name);
				// quantity
				...
				$("#editProductForm").unbind('submit').bind('submit', function() {
					...
					if(machineName == "") {
						$("#editmachineName").after('<p class="text-danger">Product Name field is required</p>');
						$('#editmachineName').closest('.form-group').addClass('has-error');
					}	else {
						// remov error text field
						$("#editmachineName").find('.text-danger').remove();
						// success out for form 
						$("#editmachineName").closest('.form-group').addClass('has-success');	  	
					}	// /else
					...
					if(machineName && quantity && rate && brandName && categoryName && productStatus) {
						// submit loading button
				...
				$("#updateimageForm").unbind('submit').bind('submit', function() {					
					// form validation
					var image = $("#editimage").val();					
					
					if(image == "") {
						$("#editimage").closest('.center-block').after('<p class="text-danger">Product Image field is required</p>');
						$('#editimage').closest('.form-group').addClass('has-error');
					}	else {
						// remov error text field
						$("#editimage").find('.text-danger').remove();
						// success out for form 
						$("#editimage").closest('.form-group').addClass('has-success');	  	
					}	// /else

					if(image) {
						// submit loading button
						$("#editimageBtn").button('loading');
						...
						$.ajax({
				          ...
				          // reload the manage student table
									...
									$.ajax({
										url: 'php_action/fetchimageUrl.php?i='+productId,
										type: 'post',
										success:function(response) {
										$("#getimage").attr('src', response);		
										}
									});																		
